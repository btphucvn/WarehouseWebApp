...
{
    public class InternalExportDocumentsController : Controller
    {
        private readonly dbwarehouseContext _context;

        public InternalExportDocumentsController(dbwarehouseContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index()
        {
            ...
            string url = "https://" + Request.Host.Value + "/api/apiDocuments?documenttype=2";
        ...
        public async Task<IActionResult> Create([Bind("DocumentId,DocumentCode,DocumentNumber,DateDocument,DocumentNumber2,DateDocument2,SupplierId,Quantity,UnitOut,TotalAmount,Note,CompanyId,UnitId,Status,CreatedDate,CreatedBy,UpdatedDate,UpdatedBy,DeleteDate,DeletedBy")] Document document)
        {
            if (ModelState.IsValid)
            {
                ...
                Sequence sequence = await sequenceController.GetSequence("NNB@" + DateTime.Today.Year.ToString() + "@" + unit.UnitCode);
                if (sequence == null)
                {
                    sequence = new Sequence();
                    sequence.SeqName = "NNB@" + DateTime.Today.Year.ToString() + "@" + unit.UnitCode;
                    sequence.SeqValue = 1;
                    sequenceController.Create(sequence);
                }
                document.DocumentType = 2;
                document.DocumentNumber = sequence.SeqValue.Value.ToString("000000") + @"/" + DateTime.Today.Year.ToString().Substring(2, 2) + @"/NNB/" + unit.UnitCode;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("DocumentId,DocumentCode,DocumentNumber,DateDocument,DocumentNumber2,UnitOut,DateDocument2,SupplierId,Quantity,TotalAmount,Note,CompanyId,UnitId,Status,CreatedDate,CreatedBy,UpdatedDate,UpdatedBy,DeleteDate,DeletedBy")] Document document)
        {
